...
function generateSchedule(weekKey, options = {}) {
  ...
  // Build availability matrix - WITH FALLBACK for employees who didn't submit
  function isAvailable(empId, dateStr, shiftId) {
    ...
    if (submission) {
      // Employee submitted availability - use their submission
      ...
      // Employee did NOT submit - use profile preferences as fallback
      // Available for all shifts matching their preference
  ...
  // Track who submitted and who uses fallback
  const notSubmitted = employees.filter(e => !weekData.submissions?.[e.id]);
  const submitted = employees.filter(e => weekData.submissions?.[e.id]);
  ...
  // Assign shifts - prefer employees who submitted over those who didn't
  for (const slot of slots) {
    ...
    const available = slot.candidates
      ...
      .sort((a, b) => {
        // First: prefer employees who submitted availability
    ...
    // First fill required roles (supports multi-role employees)
